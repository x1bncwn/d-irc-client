...
class GTKClient {
    ...
    private string[string][string] channelTopics; // server -> channel -> topic
    ...
    ~this() {
        // Clean up pipe
        if (pipeFds[0] != 0) core.sys.posix.unistd.close(pipeFds[0]);
        if (pipeFds[1] != 0) core.sys.posix.unistd.close(pipeFds[1]);
    }

    private void initializeTextTags(string bufferName) {
        if (!(bufferName in displayBuffers)) return;
    ...
    private TextTag getNicknameTag(string bufferName, string nickname, string color) {
        if (!(bufferName in nicknameTags)) return null;
        ...
        if (!(bufferName in displayBuffers)) return null;
    ...
    private TextTag getModeSymbolTag(string bufferName, char modeSymbol) {
        if (!(bufferName in modeSymbolTags)) return null;
    ...
    void setupGui() {
        ...
        if (settings) {
            ...
            settings.gtkThemeName = "Adwaita-dark";
            isDarkTheme = true;
        }
    ...
    void setupSignals() {
        ...
        // SET UP PIPE WATCH - Use IOChannel and ioAddWatch
        logToTerminal("Setting up pipe watch on fd " ~ to!string(pipeFds[0]), "DEBUG", "main");

        auto channel = IOChannel.unixNew(pipeFds[0]);
        if (channel is null) {
            logToTerminal("FAILED: IOChannel.unixNew returned null", "ERROR", "main");
            throw new Exception("Failed to create IOChannel for pipe");
        }
        logToTerminal("Created IOChannel successfully", "DEBUG", "main");

        pipeSourceId = ioAddWatch(channel, 0, IOCondition.In | IOCondition.Hup | IOCondition.Err,
            delegate bool(IOChannel channel, IOCondition condition) {
    ...
    private string getNickColor(string nickname) {
        if (!colorizeNicks) return isDarkTheme ? "#CCCCCC" : "#666666";
    ...
    private bool findServerInTree(string server, ref TreeIter iter) {
        ...
        if (channelStore.getIterFirst(childIter)) {
            do {
                ...
                string itemType = typeVal.getString();                                                                                                                    if (itemType == "server" && serverName == server) {                              iter = childIter;                                                            found = true;                                                                break;                                                                   }                                                                        } while (channelStore.iterNext(childIter));                              }                                                                            return found;                                                            }                                                                                                                                                         private bool findChannelUnderServer(TreeIter serverIter, string channel, ref TreeIter iter) {                                                                 bool found = false;                                                          TreeIter childIter;                                                          if (channelStore.iterChildren(childIter, serverIter)) {                          do {                                                                             Value val = new Value("");                                                   channelStore.getValue(childIter, 0, val);                                    string channelName = val.getString();                                        if (channelName == channel) {                                                    iter = childIter;                                                            found = true;
                    break;                                                                   }                                                                        } while (channelStore.iterNext(childIter));                              }                                                                            return found;                                                            }                                                                                                                                                         private void addServerToTree(string server) {                                    TreeIter serverIter;                                                         if (!findServerInTree(server, serverIter)) {                                     TreeIter newServerIter;                                                      channelStore.append(newServerIter, null);                                    channelStore.setValue(newServerIter, 0, new Value(server));
    ...
    private void removeChannelFromTree(string server, string channel) {              TreeIter serverIter;
        if (findServerInTree(server, serverIter)) {                                      TreeIter channelIter;
            if (findChannelUnderServer(serverIter, channel, channelIter)) {                  channelStore.remove(channelIter);
            }                                                                        }
    }                                                                        
    private void removeServerFromTree(string server) {                               TreeIter serverIter;
        if (findServerInTree(server, serverIter)) {                                      channelStore.remove(serverIter);
        }                                                                        }
                                                                                 private void startConnection(string server) {
        if (server in connections && connections[server].connected) {                    appendSystemMessage("Already connected to " ~ server ~ ".");
            return;                                                                  }
                                                                                     logToTerminal("Passing pipe write fd " ~ to!string(pipeFds[1]) ~ " to IRC thread", "DEBUG", "main");                                                      auto tid = spawn(&runIrcServer, server.strip(), thisTid, pipeFds[1]);
        ...
        currentDisplay = server;                                                     currentServer = server;
        ...
        if (!displayHistory.canFind(server)) {                                           displayHistory ~= server;
        }
    }

    private void disconnectFromServer() {                                            if (currentServer.length == 0 || !(currentServer in serverThreads)) {
            return;                                                                  }
                                                                                     auto tid = serverThreads[currentServer];
        send(tid, IrcFromGtkMessage(IrcFromGtkMessage.Type.UpdateChannels, "", "", "quit"));
                                                                                     Thread.sleep(100.msecs);
                                                                                     removeServerFromTree(currentServer);
        connections.remove(currentServer);                                           serverThreads.remove(currentServer);
                                                                                     appendSystemMessage("Disconnected from " ~ currentServer ~ ".");
        currentServer = "";                                                          currentDisplay = "System";
        textBuffer = displayBuffers["System"];                                       textView.setBuffer(textBuffer);
        scrollToEnd();                                                           }
                                                                                 private void disconnectAllServers() {
        foreach (server, tid; serverThreads) {                                           send(tid, IrcFromGtkMessage(IrcFromGtkMessage.Type.UpdateChannels, "", "", "quit"));                                                                  }
        ...
        bool gotMessage = true;                                                      while (gotMessage) {
            gotMessage = receiveTimeout(Duration.zero,                                       (WhoisReply whois) {
                    logToTerminal("Processing WHOIS reply for " ~ whois.target, "DEBUG", "main");
                    
                    // Format the WHOIS reply using the struct's format() method
                    string formatted = whois.format();
                    
                    // Append to current display
                    string display = currentDisplay;
                    if (display.length == 0) display = "System";
                    
                    if (display in displayBuffers) {
                        TextBuffer targetBuffer = displayBuffers[display];
                        TextIter insertIter;
                        targetBuffer.getEndIter(insertIter);
                        targetBuffer.insert(insertIter, formatted ~ "\n");
                        
                        // Scroll to end if this is the current display
                ...
                (IrcToGtkMessage msg) {
                    ...
                    final switch (msg.type) {                                                        case IrcToGtkType.chatMessage:
                            auto data = msg.chat;                                                        string display;
                            if (data.channel.length > 0) {                                                   display = data.channel;
                            } else {                                                                         display = data.server;
                            }                                                                            appendChatMessage(display, data.timestamp, data.prefix ~ data.rawNick, data.messageType, data.body);                                                      break;
                        case IrcToGtkType.channelUpdate:                                                 auto u = msg.channelUpdate;
                            updateChannelList(u.server, u.channel, u.action);                            break;
                        case IrcToGtkType.systemMessage:                                                 if (currentServer.length > 0) {
                                appendChatMessage(currentServer, formatTimestampNow(), "", "system", msg.systemText);                                                                 } else {
                                appendChatMessage("System", formatTimestampNow(), "", "system", msg.systemText);
                            }
                            break;
    ...
    private void handleCommand(string text) {
        ...
        else if (text.startsWith("/join ")) {                                            auto channel = text["/join ".length .. $].strip();
    ...
    void appendChatMessage(string display, string timestamp, string nickname, string type, string message) {
        ...
        switch (type) {
                ...
                if (nickname.length > 0) {
                    ...
                    if (colorizeNicks) {
                        ...
                        if (auto nickTag = getNicknameTag(display, baseNickname, nickColor)) {
                            ...
                            targetBuffer.insert(insertIter, baseNickname);                           }                                                                        } else {
                        targetBuffer.insert(insertIter, baseNickname);
                    }                                                        
                    targetBuffer.getEndIter(insertIter);
                    targetBuffer.insert(insertIter, "- " ~ message ~ "\n");
            ...
            case "kick":
            case "nick":
                if (nickname.length > 0) {                                                       if (modeSymbol != '\0') {
                        if (auto modeTag = getModeSymbolTag(display, modeSymbol)) {
                            insertWithTags(targetBuffer, insertIter, [modeSymbol].idup, modeTag);
                        } else {
                            targetBuffer.insert(insertIter, [modeSymbol].idup);
                    ...
                    if (colorizeNicks) {
                        string nickColor = getNickColor(baseNickname);
                        if (auto nickTag = getNicknameTag(display, baseNickname, nickColor)) {                                                                                        insertWithTags(targetBuffer, insertIter, baseNickname, nickTag);                                                                                      } else {
                            targetBuffer.insert(insertIter, baseNickname);                           }
                    } else {
                        targetBuffer.insert(insertIter, baseNickname);
                    }

                    targetBuffer.getEndIter(insertIter);                                         targetBuffer.insert(insertIter, " " ~ message ~ "\n");
                } else {
                    targetBuffer.insert(insertIter, message ~ "\n");
                }
                break;

            case "system":                                                                   if (display in systemMessageTags) {
                    targetBuffer.getEndIter(insertIter);
