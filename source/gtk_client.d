...
class GTKClient
{
    ...
    private string hslToHex(float h, float s, float l)
    {
        ...
        l = l < 0.0f ? 0.0f : (l > 1.0f ? 1.0f : l);                                                                                                              float c = (1.0f - abs(2.0f * l - 1.0f)) * s;
        float x = c * (1.0f - abs(fmod(h / 60.0f, 2.0f) - 1.0f));                    float m = l - c / 2.0f;
        float r, g, b;
	    ...
	    while (gotMessage)
			...
			case IrcToGtkType.channelUpdate:
                    	...
                    	case IrcToGtkType.systemMessage:
                            ...
                            {
                            	appendChatMessage(currentServer, formatTimestampNow(), "", "system", msg.systemText);
                            }
                            else
                            {
