...
class GTKClient
{
    ...
    this()
    {
        ...
        logToTerminal("Pipe created: read fd=" ~ to!string(
                pipeFds[0]) ~ ", write fd=" ~ to!string(pipeFds[1]), "DEBUG", "main");
        ...
        foreach (i, fd; pipeFds)
        {
            ...
            {
                logToTerminal("Failed to set non-blocking on fd " ~ to!string(
                        fd) ~ ", errno: " ~ to!string(errno), "WARNING", "main");
            }
        }
    ...
    void setupSignals()
    {
        ...
        pipeSourceId = ioAddWatch(channel, 0, IOCondition.In | IOCondition.Hup | IOCondition.Err,
            ...
            logToTerminal("Pipe callback fired, condition: " ~ to!string(condition),
                "DEBUG", "main");
            ...
            if (condition & IOCondition.In)
            {
                ...
                do
                {
                    ...
                    if (bytesRead > 0)
                    {
                        logToTerminal("Read " ~ to!string(bytesRead) ~ " bytes from pipe",
                            "DEBUG", "main");
                    }
        ...
        window.connectCloseRequest(delegate(Window window) {
            ...
            {
                send(tid, IrcFromGtkMessage(IrcFromGtkMessage.Type.UpdateChannels,
                    "", "", "quit"));
            }
    ...
    private void startConnection(string server)
    {
        ...
        logToTerminal("Passing pipe write fd " ~ to!string(pipeFds[1]) ~ " to IRC thread",
                "DEBUG", "main");
                ...
                case IrcToGtkType.systemMessage:
                    if (currentServer.length > 0)
                    {
                        appendChatMessage(currentServer, formatTimestampNow(),
                            "", "system", msg.systemText);
                    }
                    else
                    {
                        appendChatMessage("System", formatTimestampNow(), "",
                            "system", msg.systemText);
                    }
                    break;
    ...
    private void handleChannelTopic(string server, string channel, string topic)
    {
        ...
        {
            appendChatMessage(server, formatTimestampNow(), "", "system",
    ...
    {
        logToTerminal("Updating channel list: " ~ server ~ " -> " ~ channel ~ " " ~ action,
                "INFO", "main");
    ...
    void sendMessage()
    {
        ...
        if (currentDisplay == currentServer)
        {
            ...
            if (spacePos != -1)
            {
                ...
                send(serverThreads[currentServer],
                        IrcFromGtkMessage(IrcFromGtkMessage.Type.Message, recipient, message, ""));
            }
            else
            {
        ...
        {
            send(serverThreads[currentServer],
                    IrcFromGtkMessage(IrcFromGtkMessage.Type.Message, currentDisplay, text, ""));
        }
        else
        {
    ...
    private void handleCommand(string text)
    {
        ...
        else if (text.startsWith("/join "))
        {
            ...
            send(serverThreads[currentServer],
                    IrcFromGtkMessage(IrcFromGtkMessage.Type.UpdateChannels, channel, "", "join"));
        ...
        else if (text.startsWith("/part "))
        {
            ...
            send(serverThreads[currentServer],
                    IrcFromGtkMessage(IrcFromGtkMessage.Type.UpdateChannels, channel, "", "part"));
        ...
        else if (text.startsWith("/whois "))
        {
            ...
            {
                send(serverThreads[currentServer],
                        IrcFromGtkMessage(IrcFromGtkMessage.Type.UpdateChannels,
                            target, "", "whois"));
                appendChatMessage(currentDisplay, formatTimestampNow(), "",
                        "system", "WHOIS request sent for " ~ target);
            }
            else
            {
        ...
        else if (text.startsWith("/me "))
        {
            if (currentDisplay.startsWith("#"))
            {
                ...
                send(serverThreads[currentServer],
                        IrcFromGtkMessage(IrcFromGtkMessage.Type.Message,
                            currentDisplay, actionMsg, ""));
            }
            else
            {
        ...
        else if (text.startsWith("/nick "))
        {
            ...
            send(serverThreads[currentServer],
                    IrcFromGtkMessage(IrcFromGtkMessage.Type.Message, "", "NICK " ~ newNick, ""));
            appendChatMessage(currentServer, formatTimestampNow(), defaultNick,
                    "system", "Changing nickname to: " ~ newNick);
        }
        ...
        else
        {
            ...
            send(serverThreads[currentServer],
                    IrcFromGtkMessage(IrcFromGtkMessage.Type.Message, "", rawCommand, ""));
    ...
    {
        logToTerminal("Appending message to display " ~ display ~ ": " ~ nickname ~ " (" ~ type ~ "): " ~ message,
                "INFO", "main");
